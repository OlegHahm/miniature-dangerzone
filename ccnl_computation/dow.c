...
void *_loop(void *arg)
{
    ...
    _reset_netstats();
    dow_ts_wakeup = xtimer_now().ticks32;
    ...
    if (dow_position == 0) {
        ...
        _last_deputy_ts = xtimer_now().ticks32;
...
#if DOW_DEPUTY
                if (xtimer_now() >= (_last_deputy_ts + (_sleep_period * US_PER_SEC))) {
...
#endif
    if (!dow_sleeping) {
        dow_ts_sleep = xtimer_now().ticks32;
...
void dow_new_data(void)
{
    ...
    /* XXX: implement event */
    unsigned data = xtimer_now().ticks32;
